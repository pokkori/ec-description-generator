...
const PLANS: Record<string, string> = {
  ...
  enterprise: process.env.PAYJP_PLAN_ENT!,
};
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const cookieVal = plan === "enterprise" ? "ent" : plan === "business" ? "biz" : "1";
    res.cookies.set("premium", cookieVal, {
      httpOnly: true,
      secure: true,
      sameSite: "lax",
      maxAge: 60 * 60 * 24 * 366,
    ...
    // subscription IDを保存（auth/statusでのAPI検証に使用）
    if (sub?.id) {
