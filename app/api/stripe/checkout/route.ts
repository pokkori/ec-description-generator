...
export async function GET(req: NextRequest) {
  ...
  const origin = req.headers.get("referer")?.replace(/\/[^/]*$/, "") || "https://ec-description-generator.vercel.app";
...
export async function POST(req: NextRequest) {
  ...
  const origin = req.headers.get("origin") || "https://ec-description-generator.vercel.app";
  const session = await createSession(plan, origin);
  if (!session) return NextResponse.json({ error: "Invalid plan" }, { status: 400 });
